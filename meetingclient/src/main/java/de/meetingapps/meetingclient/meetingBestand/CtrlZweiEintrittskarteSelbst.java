...
/**
 * The Class CtrlZweiEintrittskarteSelbst.
 */
public class CtrlZweiEintrittskarteSelbst extends CtrlRootWK {
    ...
    boolean speichern_Funktion() {
        ...
        weEintrittskarte.ausgewaehlteAktion = KonstPortalAktion.ZWEI_EK_SELBST_FUER_ALLE;
                ...
                while ((zeile = br.readLine()) != null /*&& iii<45000*/) {
                    if (!zeile.isEmpty()) {
                        ...
                        if (!aktionaersnummer.equals("Barcode")) {
                            ...
                            String[] topString = new String[20];
                            topString[0] = zeileSplit[6].replace('"', ' ').trim();
                            topString[1] = zeileSplit[7].replace('"', ' ').trim();
                            topString[2] = zeileSplit[8].replace('"', ' ').trim();
                            topString[3] = zeileSplit[9].replace('"', ' ').trim();
                            topString[4] = zeileSplit[10].replace('"', ' ').trim();
                            topString[6] = zeileSplit[11].replace('"', ' ').trim();
                            topString[7] = zeileSplit[12].replace('"', ' ').trim();
                            topString[8] = zeileSplit[13].replace('"', ' ').trim();
                            ...
                            if (ek.equals("0") && vollmacht.equals("0") && briefwahlVdg.equals("0")) { // 000
                                ...
                                if (ek.equals("1")) {
                                    rc = false; // Funktion eine EK
                                } else if (ek.equals("2")) {
                                    rc = true;// Funktion zwei EKs
                                }
                            } else {
                                rc = false; // aussteuern	
                            }
    ...
    void clickedTextDateiLesen(ActionEvent event) {
        ...
        /*TODO _Anmeldeablauf Textdatei lesen bei ZweiEKs noch nicht implementiert*/
