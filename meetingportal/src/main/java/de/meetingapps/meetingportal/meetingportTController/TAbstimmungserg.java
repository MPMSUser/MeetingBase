...
@RequestScoped
@Named
public class TAbstimmungserg {

    private @Inject TAbstimmungsergSession tAbstimmungsergSession;
    ...
    /**open in aufrufender Funktion gehandelt - hier aber nicht erforderlich*/
    public void init(boolean pFuelleFuerMonitoring) {
        ...
        int lfdNr = 0;
        if (tAuswahlSession.isAbstimmungsergebnisseAktiv() == true || pFuelleFuerMonitoring==true) {
            for (int i = 1; i <= eclParamM.getParam().paramPortal.abstimmungsergLetzteNr; i++) {
                EclAbstTeilButtonM lAbstTeilButtonM = new EclAbstTeilButtonM();
                lAbstTeilButtonM.setLfdNr(lfdNr);
                lfdNr++;
                if (eclParamM.getParam().paramPortal.abstimmungsergLetzteNr == 1) {
                    lAbstTeilButtonM.setButtontext1Vorne("1426");
                    lAbstTeilButtonM.setButtontext1Ziffer("");   
                    lAbstTeilButtonM.setButtontext1Hinten("-");
                    lAbstTeilButtonM.setDateiname1("Abstimmungserg" + Integer.toString(i));
                } else {
                    lAbstTeilButtonM.setButtontext1Vorne("1427");
                    lAbstTeilButtonM.setButtontext1Ziffer(Integer.toString(i));   
                    lAbstTeilButtonM.setButtontext1Hinten("1428");
                    lAbstTeilButtonM.setDateiname1("Abstimmungserg" + Integer.toString(i));
                }
        ...
        tAbstimmungsergSession.setAbsTeilButtonList(absTeilButtonList);
    }

    public void doAnzeige(EclAbstTeilButtonM lAbstTeilButtonM) {
        if (!tSessionVerwaltung.pruefeStart(KonstPortalView.ABSTIMMUNGSERGEBNISSE)) {
            return;
        }
        boolean anzeigenMoeglich=true;
        eclDbM.openAll();
        int lPortalFunktion=KonstPortalFunktionen.portalFunktionZuView(KonstPortalView.ABSTIMMUNGSERGEBNISSE);
        ...
        tSessionVerwaltung.setzeEnde(KonstPortalView.ABSTIMMUNGSERGEBNISSE);
        return;
    }
