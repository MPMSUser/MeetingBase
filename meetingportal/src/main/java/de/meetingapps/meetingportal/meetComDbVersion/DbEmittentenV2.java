...
public class DbEmittentenV2 {

    //	private Connection verbindung=null;
    private DbBundle dbBundle = null;

    /*************************Initialisierung***************************/
    public DbEmittentenV2(DbBundle pDbBundle) {
    ...
    public int alterTableV2_V3() {
        ...
        rc = lDbLowLevel.rawOperation("ALTER TABLE db_meetingcomfort.tbl_emittenten "
                + "ADD COLUMN portalStandard INT(11) NULL DEFAULT 1 AFTER portalAktuellAktiv;");
        if (rc < 0) {
            return rc;
        }

        rc = lDbLowLevel.rawOperation("ALTER TABLE db_meetingcomfort.tbl_emittenten "
                + "ADD COLUMN hvCode CHAR(10)  DEFAULT NULL AFTER dbArt; ");
        if (rc < 0) {
            return rc;
        }
