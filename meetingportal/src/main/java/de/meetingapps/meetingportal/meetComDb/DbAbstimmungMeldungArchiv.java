...
/*Kopie von DbAbstimmungMeldung.
 * Änderungen sind markiert, so dass nach der Saison ein Zusammenführen wieder möglich ist.
 * 
 * Folgende Änderungen wurden global durchgeführt:
 * 
 * tbl_abstimmungmeldungArchiv=> tbl_abstimmungmeldungArchivArchiv
 */
public class DbAbstimmungMeldungArchiv  extends DbRootExecute {
    ...
    public DbAbstimmungMeldungArchiv(DbBundle datenbankbundle) {
    ...
    public int createTable() {
        ...
        String hString = "CREATE TABLE " + dbBundle.getSchemaMandant() + "tbl_abstimmungmeldungArchiv ( "
        ...
        hString = hString + "PRIMARY KEY (`meldungsIdent`,`mandant`) " + ") ";
    ...
    public int deleteAll() {
        ...
        /*tbl_abstimmungmeldungArchiv*/
        try {

            String sql1 = "DELETE FROM " + dbBundle.getSchemaMandant() + "tbl_abstimmungmeldungArchiv where mandant=?;";
    ...
    public int updateMandant() {
        return dbBundle.dbLowLevel.rawUpdateMandant(dbBundle.getSchemaMandant() + "tbl_abstimmungmeldungArchiv");
    }
    ...
    public int insert(EclAbstimmungMeldung lAbstimmungMeldung) {
        ...
        try {
            ...
            String sql1 = "INSERT INTO " + dbBundle.getSchemaMandant() + "tbl_abstimmungmeldungArchiv " + "(" + "mandant,  "
    ...
    public int lese_Ident(int lIdent) {
        ...
        try {
            ...
            String sql = "";
            sql = "SELECT am.* from " + dbBundle.getSchemaMandant() + "tbl_abstimmungmeldungArchiv am where "
    ...
    public int update(EclAbstimmungMeldung lAbstimmungMeldung) {
        ...
        try {
            ...
            String sql = "UPDATE " + dbBundle.getSchemaMandant() + "tbl_abstimmungmeldungArchiv SET " + "mandant=?, "
                    + "meldungsIdent=?,  stimmen=?, " + "gattung=?, aktiv=?, erteiltAufWeg=?, zeitstempelraw=? ";
            for (int i = 0; i < 200; i++) {
    ...
    public int update_allPosition(int position) {
        try {
            String sql = "UPDATE " + dbBundle.getSchemaMandant() + "tbl_abstimmungmeldungArchiv SET ";
    ...
    /***************Ab hier neue Funktionen nur in dieser Klasse********/
    public int initialisiereArchivTable() {
        DbLowLevel dbLowLevel=new DbLowLevel(dbBundle);
        boolean brc=dbLowLevel.checkTableVorhandenNew(dbBundle.getSchemaMandantOhnePunkt(), "tbl_abstimmungmeldungArchiv");
        if (brc==true) {
            deleteAll();
        }
        else {
