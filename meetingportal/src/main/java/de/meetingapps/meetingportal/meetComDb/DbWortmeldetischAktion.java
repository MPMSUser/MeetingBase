...
public class  DbWortmeldetischAktion extends DbRoot<EclWortmeldetischAktion> {

//    private int logDrucken=10;
    
    public DbWortmeldetischAktion(DbBundle pDbBundle) {
        super(pDbBundle);
    }
    ...
    String getCreateString() {
        ...
        String createString = "CREATE TABLE " + getSchema() + getTableName()+" ( "
                ...
                + "`vonStatusBezeichnung` varchar(60) NOT NULL DEFAULT '', " 
                + "`zuStatusBezeichnung` varchar(60) NOT NULL DEFAULT '', " 
                + "`aktionBezeichnung` varchar(60) NOT NULL DEFAULT '', " 
                + "`nurWennBedingungErfuellt` varchar(60) NOT NULL DEFAULT '', " 
                + "`reihenfolge` int NOT NULL DEFAULT 0, " 
                + "PRIMARY KEY (`setNr`, `viewBezeichnung`, `vonStatusBezeichnung`, `zuStatusBezeichnung`, `aktionBezeichnung`) " + ") ";
        //      @formatter:on
        return createString;
    }
    ...
    @Override
    String getTableName() {
         return "tbl_wortmeldetischAktion";
    }
    ...
    @Override
    EclWortmeldetischAktion decodeErgebnis(ResultSet pErgebnis) {
        EclWortmeldetischAktion lEclReturn = new EclWortmeldetischAktion();

        try {
            lEclReturn.setNr = pErgebnis.getInt("setNr");
            lEclReturn.viewBezeichnung = pErgebnis.getString("viewBezeichnung");
            lEclReturn.vonStatusBezeichnung = pErgebnis.getString("vonStatusBezeichnung");
            lEclReturn.zuStatusBezeichnung = pErgebnis.getString("zuStatusBezeichnung");
            lEclReturn.aktionBezeichnung = pErgebnis.getString("aktionBezeichnung");
            lEclReturn.nurWennBedingungErfuellt = pErgebnis.getString("nurWennBedingungErfuellt");
            lEclReturn.reihenfolge = pErgebnis.getInt("reihenfolge");
    ...
    @Override
    void fuellePreparedStatementKomplett(PreparedStatement pPStm, int pOffset, EclWortmeldetischAktion pEcl) {
...
//          @formatter:off
            ...
            pPStm.setString(pOffset, pEcl.vonStatusBezeichnung);pOffset++;
            pPStm.setString(pOffset, pEcl.zuStatusBezeichnung);pOffset++;
            pPStm.setString(pOffset, pEcl.aktionBezeichnung);pOffset++;
            pPStm.setString(pOffset, pEcl.nurWennBedingungErfuellt);pOffset++;
            pPStm.setInt(pOffset, pEcl.reihenfolge);pOffset++;
...
//  @formatter:off
    private final String[] felder= {"setNr", 
            "viewBezeichnung", "vonStatusBezeichnung", "zuStatusBezeichnung",
            "aktionBezeichnung", "nurWennBedingungErfuellt", "reihenfolge"
            }; 
//  @formatter:on
    ...
    @Override
    public int insert(EclWortmeldetischAktion pEcl) {
        initFelder();
    ...
    /**Liest alle Demo-Kennungen ein*/
     public int readAll(int pSetNr) {
        ...
        try {
            ...
            lSql = lSql + "WHERE setNr=? ORDER BY reihenfolge;";
...
//     * 
//     */
//    public int update(EclWortmeldetischAktion pEcl) {
//        int ergebnis=0;
//        initFelder();
//
//        try {
